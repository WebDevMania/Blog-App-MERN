...
const Register = () => {
  ...
  const handleRegister = async(e) => {
    e.preventDefault()
    if(username === "" || email === "" || password === "" || confirmPass === "") return
    if(confirmPass !== password) throw new Error("Passwords are not equal!")
  ...
  return (
    <div className='h-screen w-screen flex items-center justify-center bg-[#6a6ac2] absolute top-0 left-0 z-50'>
      <div className='flex flex-col items-center justify-center bg-[#f4f4f4] p-8 border border-[#a09292] rounded-[30px] h-[475px] w-[375px]'>
        <h2 className='mb-5 text-[#3a3636] font-bold text-[22px]'>Register</h2>
        <form onSubmit={handleRegister} className='flex flex-col items-center justify-center gap-5'>
          <label htmlFor='username'>
            <input placeholder='Username...' onChange={(e) => setUsername(e.target.value)} className='py-2 px-5 bg-transparent border-2 border-[#adabab] rounded-[0.75rem] focus:outline-none focus:border-[#333]' type="text" id='username'/>
          </label>
          <label htmlFor='email'>
          ...
          <label htmlFor='confirmPass'>
            <input placeholder='Confirm password...' onChange={(e) => setConfirmPass(e.target.value)} className='py-2 px-5 bg-transparent border-2 border-[#adabab] rounded-[0.75rem] focus:outline-none focus:border-[#333]' type="password" id='confirmPass'/>
          </label>
          ...
          <Link className='text-inherit no-underline' to="/auth/login">
            Already have an account? <p className='w-[150px] border-b border-[#333] pb-1 my-1 mx-auto text-center hover:text-[#666] hover:border-[#666]'>Login now</p>
          </Link>
        </form>
      </div>
    </div>
